...
async function check_file_exists(file_path) {
  return asyncfs
    .access(file_path, fs.constants.F_OK)
    .then(() => {
      return true;
    })
    .catch(() => {
...
async function get_app_server() {
  ...
  app.post(
    "/page_callback",
    upload.none(),
    validate({ body: CollectedPagesCallbackSchema }),
    async (req, res) => {
  ...
  app.post(
    ...
    validate({ body: JSCallbackSchema }),
    async (req, res) => {
      res.set("Access-Control-Allow-Origin", "*");
      res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
      res.set("Access-Control-Allow-Headers", "Content-Type, X-Requested-With");
      ...
      if (process.env.DISCORD_WEBHOOK_URL) {
        await notification.sendDiscordWebhook(payload_fire_data).then().catch(err => {
					console.log(err)
				});
      }
    }
  ...
  app.get("/health", async (req, res) => {
    try {
      ...
      console.error(
        "An error occurred when testing the database connection (/health):",
        error
      );
      res
        .status(500)
